	...
	<nav class="navbar navbar-expand-lg navbar-dark bg-success"  style="position: sticky; top: -1px; z-index: 1;">
	    ...
	    <div class="collapse navbar-collapse" id="navbarSupportedContentLG">
	        <ul class="navbar-nav mr-auto">
			   ...
			   <%} %>
	           <li id="na" class="nav-item active"><a class="nav-link" href="ThanhtoanController">Thanh toán</a></li>
			   <li id="na" class="nav-item"><a class="nav-link" href="LsmuahangController">Lịch sử mua hàng</a></li>
	        </ul>
	        <form class="form-inline my-2 my-lg-0" method="post" action="ThanhtoanController" style="display: none;">
	...
	<div class="container-fluit">
		<div class="row" style="background-color: palegreen; margin-right: 0px;">
			<div class="col-xs-8 col-sm-9 col-md-9">
				...
				<%
				hoadonbo hd = (hoadonbo)session.getAttribute("hd");
				String tendn = (String)session.getAttribute("kh");
				if(hd!=null){
					if(tendn!=null)
						for(hoadonbean g: hd.getds(tendn)){
				%>
					<tr>
						<td><%=g.getTensach() %></td>
						<td><%=g.getTacgia() %></td>
						<td><%=g.getGia() %></td>
						<td>
							<form method="post" action="ThanhtoanController?action=update&ma=<%=g.getMasach()%>">
								Số lượng: <input type="number" name="txtsl" value=<%=g.getSoluong() %> style="width:35px;"> 
								<input type="submit" name="but1" value="Sửa" class="btn btn-primary btn-sm" style="margin-top: -3px;">
							</form>
						</td>
						<td>
							<a href="ThanhtoanController?action=delete&ma=<%= g.getMasach() %>">
								<button type="button" class="btn btn-danger btn-sm">Bỏ</button>
							</a>
						</td>
					</tr>
				<%} }%>
			...
			<div class="col-xs-4 col-sm-3 col-md-3">
				...
				<%if(hd!=null){ %>
						Thành tiền: <span style="color: blue;"><%=hd.Tong(tendn) %> VNĐ</span>
				<%} %>
				</div>
				<div style="float: right; margin-right: 10vw;">
					<%if(session.getAttribute("acc")==null || session.getAttribute("acc").equals("")) { %>
						<button id="btn4"type="button" class="btn btn-info btn-sm" style="width: 90px;">Thanh toán</button>
					<%}
					if(session.getAttribute("acc").equals("kh")){ %>
						<a href="ThanhtoanController?action=ttHD"><button type="button" class="btn btn-info btn-sm" style="width: 90px;">Thanh toán</button></a>
					<%} %>
				</div>
				<script>
